...
import { adminReducer, createUserAdmin } from '@/redux/reducer/admin.reducer';

export interface FormAddUserProps {
    closeDrawer: () => void
}

const FormAddUser: React.FC<FormAddUserProps> = ({ closeDrawer }) => {
    const { list_wilayah, list_role } = useSelector(adminReducer)
    ...
    const onSubmit = async (data: ICreateUser) => {
        const response = (await dispatch(createUserAdmin(data))) as any
        ...
        <div className='relative p-8 h-full grid grid-rows-12 '>
            ...
            <form className="flex flex-col gap-6 p-4  overflow-auto h-full row-span-10">
                <div>
                    ...
                    <Select size="lg" onChange={val => setValue("regions_ids", val || "", { shouldValidate: true })} error={errors.regions_ids && true}>
                        {
                ...
                <div>
                    ...
                    <div className="relative flex">
                        <Select size="lg" onChange={val => setValue("role", val || "", { shouldValidate: true })} error={errors.role && true}>
                            {

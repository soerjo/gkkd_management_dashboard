...
import createReportBlesscomn from "@/common/validator/create-report-blesscomn.validator";
import { yupResolver } from "@hookform/resolvers/yup";
import { useForm } from "react-hook-form";
import { ICreateReportBlesscomn } from "@/common/interfaces/create-report-blesscomn.interface";
import { toast } from "react-toastify";

export interface FormAddReportBlesscomnProps {
    closeDrawer: () => void;
}

const FormAddReportBlesscomn: React.FC<FormAddReportBlesscomnProps> = ({ closeDrawer }) => {
    const {
        setValue,
        register,
        handleSubmit,
        formState: { errors },
        watch
    } = useForm<ICreateReportBlesscomn>({ resolver: yupResolver(createReportBlesscomn), reValidateMode: "onChange" });

    const onSubmit = async (data: ICreateReportBlesscomn) => {
        toast.warn(JSON.stringify(data), { theme: "colored" })
    }
    ...
    return (
        <div className="relative p-8 h-full grid grid-rows-12">
            ...
            <form className="flex flex-col gap-6 p-4 overflow-auto h-full row-span-10">
                <div>
                    ...
                    <div className="relative flex">
                        <Select size="lg" onChange={val => setValue("blesscomn_id", val ? val : "", { shouldValidate: true })} error={errors.blesscomn_id && true}>
                            <div className="sticky w-full min-w-[200px] h-10">
                                <input
                            ...
                            <Option value="12345678">Material Tailwind HTML</Option>
                    ...
                    {errors.blesscomn_id && <div className="text-red-500 text-sm">{errors.blesscomn_id.message}</div>}
                </div>
                <div>
                    <Typography variant="small" color="blue-gray" className="mb-1 font-medium capitalize">
                        tanggal
                    ...
                    <Datepicker
                        ...
                        useRange={false}
                        asSingle={true}
                        value={{ startDate: watch("date"), endDate: watch("date") }}
                        onChange={(val) => val?.startDate && setValue("date", new Date(val.startDate), { shouldValidate: true })}
                        inputClassName={"border-[1px] border-blue-gray-200 rounded-lg w-full h-full p-3"}
                    />
                ...
                <div>
                    ...
                    <Input
                        ...
                        label=""
                        {...register('total_male')}
                        error={errors.total_male && true}
                    />
                    {errors.total_male && <div className="text-red-500 text-sm">{errors.total_male.message}</div>}
                </div>
                <div>
                    <Typography variant="small" color="blue-gray" className="mb-1 font-medium capitalize">
                        total perempuan
                    ...
                    <Input
                        ...
                        error={errors.total_female && true}
                    />
                    {errors.total_female && <div className="text-red-500 text-sm">{errors.total_female.message}</div>}
                </div>
                <div>
                    <Typography variant="small" color="blue-gray" className="mb-1 font-medium capitalize">
                        total orang baru
                    ...
                    <Input
                        ...
                        placeholder="name@mail.com"
                        label=""
                        {...register('new', { value: 0 })}
                        error={errors.new && true}
                    />
                    {errors.new && <div className="text-red-500 text-sm">{errors.new.message}</div>}
                </div>
                ...
                <div>
                    ...
                    <Input
                        value={Number(watch("total_female") || 0) + Number(watch("total_male") || 0)}
                        size="lg"
                        type="number"
                        placeholder="name@mail.com"
                        disabled
            ...
            <div className="absolute flex gap-6 bottom-0 right-0 p-8">
                <IconButton onClick={closeDrawer} className="bg-[#ffdede] p-6 rounded-full">
...
export default FormAddReportBlesscomn;
